...
pub fn softcap_f32(
    ...
    pre.validate()?;
    if pre.dtype != crate::tensor::DType::F32 || !Arc::ptr_eq(rt, pre.runtime()) ||
        !softcap.is_finite() || softcap <= 0.0 || pre.numel() > u32::MAX as usize {
        return Err("softcap requires f32, matching runtime, uint count, and a finite positive cap".into());
    }
